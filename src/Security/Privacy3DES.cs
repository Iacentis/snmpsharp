...
public class Privacy3DES : IPrivacyProtocol
{
        ...
        byte[] privacyParameters)
    {
        ...
        {
            throw new SnmpPrivacyException(ex,
                "Exception was thrown while TripleDES privacy protocol was decrypting data.");
        }
	...
	private byte[] GetSalt(int engineBoots)
    {
        ...
        var s = NextSalt();
        var eb = BitConverter.GetBytes(engineBoots);
        // C# is little endian so reverse the values
        salt[3] = eb[0];
        salt[2] = eb[1];
        salt[1] = eb[2];
        salt[0] = eb[3];
        var sl = BitConverter.GetBytes(s);
        salt[7] = sl[0];
        salt[6] = sl[1];
        salt[5] = sl[2];
        salt[4] = sl[3];
	...
	private byte[] GetIV(byte[] privacyKey, byte[] salt)
    {
        ...
        var iv = new byte[8];
        for (var i = 0; i < iv.Length; i++) iv[i] = (byte)(salt[i] ^ privacyKey[24 + i]);
        return iv;
    }
}